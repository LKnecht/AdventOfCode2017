/*
  Advent of Code 2017
  Day 5
  Jump around in an array of jump steps until you leave the array.
  On every position increment the value before jumping.
  In part 2 decrease the value if it is greater than 2
*/

#include <stdio.h>
#include <string.h> // provides declaration of memcpy

// duplicate an array (looked up at stackoverflow)
int * duplicate(int const * src, size_t length) {
  int * p;
  if ((p = malloc(length * sizeof(int))) != NULL) {
    memcpy(p, src, length * sizeof(int));
    return p;
  }
}

int solve(int * jumps, size_t length, int part) {
  int tmp;
  int stepCounter = 0;
  int position = 0;
  while (position > -1 && position < length) {
    tmp = jumps[position];
    if (part == 1 || jumps[position] < 3) {
      jumps[position] += 1;
    } else {
      jumps[position] -= 1;
    }
